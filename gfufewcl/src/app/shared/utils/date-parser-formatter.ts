...
export class DateParserFormatter extends NgbDateParserFormatter {
  ...
  parse(value: string): NgbDateStruct {
    if (value) {
      ...
      if (isNumber(dateParts[0]) && isNumber(dateParts[1]) && isNumber(dateParts[2])) {
        ...
        anio = this.yyIndex === 0 ? (dateParts[0].length < 2) ? 0 :
        toInteger(dateParts[0]) + this.anioSumar :
        this.yyIndex === 1 ? (dateParts[1].length < 2) ? 0 :
        toInteger(dateParts[1]) + this.anioSumar :
        this.yyIndex === 2 ? (dateParts[2].length < 2) ? 0 :
        toInteger(dateParts[2]) + this.anioSumar : 0;
      }
  ...
  format(date: NgbDateStruct): string {
    ...
    if (date) {
      ...
      stringDate = (stringDay) ? this.mask.replace('dd', stringDay) : this.ddIndex === 0 ?
      this.mask.replace('dd' + this.separator, '') : this.mask.replace(this.separator + 'dd', '');
      stringDate = (stringMonth) ? stringDate.replace('MM', stringMonth) :
      this.mmIndex === 0 ? stringDate.replace('MM' + this.separator, '') : stringDate.replace(this.separator + 'MM', '');
      if (this.anioSumar) {
